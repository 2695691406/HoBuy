...
public class SphinxClient {
	...
	private byte[] _GetResponse(Socket sock) {
		...
		try {
			...
			if (len != 0) {
				...
				/* build error message */
				_error = "failed to read searchd response (status=" + status + ", ver=" + ver + ", len=" + len
						+ ", trace=" + sTrace + ")";
			} else {
	...
	public void SetMatchMode(int mode) throws SphinxException {
		myAssert(
				mode == SPH_MATCH_ALL || mode == SPH_MATCH_ANY || mode == SPH_MATCH_PHRASE || mode == SPH_MATCH_BOOLEAN
						|| mode == SPH_MATCH_EXTENDED || mode == SPH_MATCH_FULLSCAN || mode == SPH_MATCH_EXTENDED2,
	...
	public void SetRankingMode(int ranker) throws SphinxException {
		myAssert(ranker == SPH_RANK_PROXIMITY_BM25 || ranker == SPH_RANK_BM25 || ranker == SPH_RANK_NONE
	...
	public void SetSortMode(int mode, String sortby) throws SphinxException {
		myAssert(
				mode == SPH_SORT_RELEVANCE || mode == SPH_SORT_ATTR_DESC || mode == SPH_SORT_ATTR_ASC
						|| mode == SPH_SORT_TIME_SEGMENTS || mode == SPH_SORT_EXTENDED || mode == SPH_SORT_EXPR,
				"unknown mode value; use one of the available SPH_SORT_xxx constants");
		myAssert(mode == SPH_SORT_RELEVANCE || (sortby != null && sortby.length() > 0),
	...
	public void SetGroupBy(String attribute, int func, String groupsort) throws SphinxException {
		myAssert(
				func == SPH_GROUPBY_DAY || func == SPH_GROUPBY_WEEK || func == SPH_GROUPBY_MONTH
	...
	public void SetOverride(String attrname, int attrtype, Map values) throws SphinxException {
		...
		myAssert(
				attrtype == SPH_ATTR_INTEGER || attrtype == SPH_ATTR_TIMESTAMP || attrtype == SPH_ATTR_BOOL
	...
	public int AddQuery(String query, String index, String comment) throws SphinxException {
		...
		try {
			...
			/* anchor point */
			if (_latitudeAttr == null || _latitudeAttr.length() == 0 || _longitudeAttr == null
